from flask import Flask, render_template, request, jsonify, session, redirect, url_for, send_file
from dotenv import load_dotenv
from concurrent.futures import ThreadPoolExecutor, as_completed
from functools import wraps
import pandas as pd
import requests
import os
from datetime import datetime
import json
import pytz
from apscheduler.schedulers.background import BackgroundScheduler
from apscheduler.triggers.date import DateTrigger
from history import (
    save_history_record, load_history, get_notification_type_name,
    update_schedule_record, ensure_data_directory
)

# ── 環境變數 ────────────────────────────────────────────────────────────────

dotenv_path = os.path.join(os.getcwd(), '.env')
load_dotenv(dotenv_path=dotenv_path)
print("Loaded .env file :", os.path.abspath(dotenv_path))

API_URL = os.getenv("PUSH_API_URL")
API_KEY = os.getenv("PUSH_API_KEY")
if not API_URL or not API_KEY:
    raise ValueError("沒有設定環境變數")

HEADERS = {
    "push-api-key": API_KEY,
    "Content-Type": "application/json"
}

# ── 常數 ────────────────────────────────────────────────────────────────────

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
SCHEDULED_NOTIFICATIONS_FILE = 'data/scheduled_notifications.json'
TAIWAN_TZ = pytz.timezone('Asia/Taipei')
MAX_WORKERS = 10

# ── Flask 應用 ───────────────────────────────────────────────────────────────

app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET_KEY", "your-secret-key-change-this-in-production")


@app.context_processor
def inject_user():
    return {'current_user': session.get('display_name', '')}

# ── 排程推播資料管理 ─────────────────────────────────────────────────────────

def load_scheduled_notifications():
    """載入排程推播資料"""
    if not os.path.exists(SCHEDULED_NOTIFICATIONS_FILE):
        return {"schedules": []}
    try:
        with open(SCHEDULED_NOTIFICATIONS_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception as e:
        print(f"載入排程資料錯誤: {e}")
        return {"schedules": []}


def save_scheduled_notifications(data):
    """保存排程推播資料"""
    try:
        with open(SCHEDULED_NOTIFICATIONS_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"保存排程資料錯誤: {e}")


def add_scheduled_notification(schedule_id, notification_type, filename, schedule_datetime, notification_tasks):
    """新增排程推播"""
    data = load_scheduled_notifications()
    data['schedules'].append({
        'id': schedule_id,
        'notification_type': notification_type,
        'filename': filename,
        'schedule_datetime': schedule_datetime.isoformat(),
        'notification_tasks': notification_tasks,
        'created_at': datetime.now(TAIWAN_TZ).isoformat(),
        'status': 'pending'
    })
    save_scheduled_notifications(data)


def remove_scheduled_notification(schedule_id):
    """移除已執行的排程"""
    data = load_scheduled_notifications()
    data['schedules'] = [s for s in data['schedules'] if s['id'] != schedule_id]
    save_scheduled_notifications(data)

# ── 推播發送核心邏輯 ─────────────────────────────────────────────────────────

def send_notification(employee_id, amount, body_text, file_title):
    """發送單一推播通知"""
    try:
        html_body = (
            f'<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8">'
            f'<title>{file_title}</title>'
            f'<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=yes">'
            f'<style>'
            f'body {{margin:0;font-family:Arial,sans-serif;background-color:#f9f9f9;line-height:1.6;color:#333;}}'
            f'.content-wrap {{background-color:white;max-width:800px;margin:0 auto;padding:16px;'
            f'box-shadow:0 2px 6px rgba(0,0,0,0.1);border-radius:8px;}}'
            f'.amount {{color:#008a00;font-weight:bold;}}'
            f'a {{color:#0066cc;text-decoration:none;}} a:hover {{text-decoration:underline;}}'
            f'@media (max-width:600px) {{.content-wrap {{padding:12px;}}}}'
            f'@media (min-width:601px) and (max-width:1024px) {{.content-wrap {{padding:20px;}}}}'
            f'@media (min-width:1025px) {{.content-wrap {{padding:32px;}}}}'
            f'</style></head><body>'
            f'<div class="content-wrap"> {body_text} '
            f'<strong><span class="amount">{amount} 元</span>。</strong>'
            f'</div></body></html>'
        )

        data = {
            "title": file_title,
            "body": html_body,
            "type": "text",
            "bodyType": "html",
            "projects": ["Portal-APP"],
            "platforms": ["iOS", "Android"],
            "inbox": "user",
            "recipients": [employee_id]
        }

        response = requests.post(API_URL, json=data, headers=HEADERS, timeout=30)

        if 200 <= response.status_code < 300:
            return {"employee_id": employee_id, "status_code": response.status_code, "success": True}

        try:
            error_message = response.json().get('message', response.text)
        except Exception:
            error_message = response.text

        return {
            "employee_id": employee_id,
            "status_code": response.status_code,
            "error": f"API 錯誤 {response.status_code}: {error_message}",
            "success": False
        }

    except requests.exceptions.Timeout:
        return {"employee_id": employee_id, "status_code": 408, "error": "請求超時（超過 30 秒）", "success": False}
    except requests.exceptions.RequestException as e:
        return {"employee_id": employee_id, "status_code": 500, "error": f"網路請求錯誤: {e}", "success": False}
    except Exception as e:
        return {"employee_id": employee_id, "status_code": 500, "error": f"未預期的錯誤: {e}", "success": False}


def send_bulk(notification_tasks):
    """並發發送所有推播，返回 responses 列表"""
    responses = []
    #推播任務數量較多時使用 ThreadPoolExecutor 進行並發處理，提升效率
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_task = {
            executor.submit(
                send_notification,
                task['employee_id'], task['amount'],
                task['body_text'], task['file_title']
            ): task for task in notification_tasks
        }
        for future in as_completed(future_to_task):
            task = future_to_task[future]
            try:
                responses.append(future.result())
            except Exception as e:
                responses.append({
                    "employee_id": task['employee_id'],
                    "status_code": 500,
                    "error": f"執行錯誤: {e}",
                    "success": False
                })
    return responses

# ── 排程執行 ─────────────────────────────────────────────────────────────────

def execute_scheduled_notification(schedule_id):
    """執行排程的推播任務"""
    print(f"\n========== 開始執行排程推播 ==========")
    print(f"排程ID: {schedule_id}, 執行時間: {datetime.now(TAIWAN_TZ)}")

    data = load_scheduled_notifications()
    schedule = next((s for s in data['schedules'] if s['id'] == schedule_id), None)
    if not schedule:
        print(f"找不到排程 {schedule_id}")
        return

    notification_tasks = schedule['notification_tasks']
    notification_type = schedule['notification_type']
    filename = schedule['filename']

    print(f"準備發送 {len(notification_tasks)} 則排程推播...")
    responses = send_bulk(notification_tasks)

    success_count = sum(1 for r in responses if r.get('success', False))
    fail_count = len(responses) - success_count
    print(f"排程推播完成: 成功 {success_count} 則, 失敗 {fail_count} 則")

    # 更新歷史記錄
    updated = update_schedule_record(schedule_id, {
        'status': 'completed',
        'executed_at': datetime.now(TAIWAN_TZ).isoformat(),
        'summary': {'total': len(responses), 'success': success_count, 'failed': fail_count},
        'details': responses
    })

    # 若找不到舊記錄則建立新記錄
    if not updated:
        save_history_record(
            notification_type=notification_type,
            filename=filename,
            total=len(responses),
            success=success_count,
            failed=fail_count,
            responses=responses,
            is_scheduled=True,
            scheduled_time=schedule['schedule_datetime'],
            schedule_id=schedule_id
        )

    remove_scheduled_notification(schedule_id)
    print(f"已移除排程 {schedule_id}")


def restore_scheduled_jobs():
    """從檔案恢復排程任務（應用重啟時）"""
    data = load_scheduled_notifications()
    now = datetime.now(TAIWAN_TZ)
    for schedule in data['schedules']:
        schedule_id = schedule['id']
        schedule_datetime = datetime.fromisoformat(schedule['schedule_datetime'])
        if schedule_datetime > now:
            scheduler.add_job(
                execute_scheduled_notification,
                trigger=DateTrigger(run_date=schedule_datetime),
                args=[schedule_id],
                id=schedule_id,
                replace_existing=True
            )
            print(f"已恢復排程: {schedule_id}, 預計執行時間: {schedule_datetime}")
        else:
            print(f"排程 {schedule_id} 已過期，跳過")

# ── 初始化排程器 ─────────────────────────────────────────────────────────────

scheduler = BackgroundScheduler(timezone=TAIWAN_TZ)
scheduler.start()
restore_scheduled_jobs()

# ── Keycloak 驗證 ────────────────────────────────────────────────────────────

KEYCLOAK_TOKEN_URL = "https://kc.futsu.com.tw/realms/staff/protocol/openid-connect/token"
KEYCLOAK_USERINFO_URL = "https://kc.futsu.com.tw/realms/staff/protocol/openid-connect/userinfo"
KEYCLOAK_CLIENT_ID = os.getenv("KEYCLOAK_CLIENT_ID", "oauth2-proxy")
KEYCLOAK_CLIENT_SECRET = os.getenv("KEYCLOAK_CLIENT_SECRET", "")


def login_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not session.get('logged_in'):
            return redirect(url_for('login'))
        return f(*args, **kwargs)
    return decorated_function

# ── 路由 ─────────────────────────────────────────────────────────────────────

@app.route('/login', methods=['GET', 'POST'])
def login():
    if session.get('logged_in'):
        return redirect(url_for('index'))
    error = None
    if request.method == 'POST':
        username = request.form.get('username', '').strip()
        password = request.form.get('password', '')
        try:
            token_resp = requests.post(
                KEYCLOAK_TOKEN_URL,
                data={
                    "grant_type": "password",
                    "client_id": KEYCLOAK_CLIENT_ID,
                    "client_secret": KEYCLOAK_CLIENT_SECRET,
                    "username": username,
                    "password": password,
                    "scope": "openid profile",
                },
                timeout=10
            )
            if token_resp.status_code != 200:
                error = '帳號或密碼錯誤，請重新輸入。'
            else:
                access_token = token_resp.json().get("access_token")
                userinfo_resp = requests.get(
                    KEYCLOAK_USERINFO_URL,
                    headers={"Authorization": f"Bearer {access_token}"},
                    timeout=10
                )
                # 嘗試從 userinfo 響應中獲取使用者資訊，如果失敗則使用登入的 username 作為 fallback
                userinfo = userinfo_resp.json() if userinfo_resp.status_code == 200 else {}
                # 檢查使用者是否屬於 EWC(福委會) 群組，沒有則拒絕登入
                if 'EWC' not in userinfo.get('groups', []):
                    error = '您沒有權限存取此系統，請聯繫管理員。'
                else:
                    session['logged_in'] = True
                    session['username'] = userinfo.get('preferred_username', username)
                    session['display_name'] = userinfo.get('name', userinfo.get('given_name', username))
                    return redirect(url_for('index'))
        except requests.exceptions.RequestException:
            error = '無法連線至驗證伺服器，請稍後再試。'
    return render_template('login.html', error=error)

# 登出路由，清除 session 並重定向到登入頁面
@app.route('/logout')
def logout():
    session.clear()
    return redirect(url_for('login'))

@app.route('/')
@login_required
def index():
    return render_template('index.html')

# 下載推播標準格式範本
@app.route("/download_template")
@login_required
def download_template():
    file_path = os.path.join(BASE_DIR, "data", "推播標準格式.xlsx")
    return send_file(
        file_path,
        as_attachment=True,
        download_name="推播標準格式.xlsx",
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )

# 歷史記錄
@app.route('/history')
@login_required
def history():
    """顯示歷史記錄列表"""
    records = load_history()
    records.reverse()
    for record in records:
        record['type_name'] = get_notification_type_name(record.get('notification_type', ''))
    return render_template('history.html', records=records)

# 歷史記錄詳細資訊
@app.route('/result')
@login_required
def push_result():
    """顯示推播結果（POST-Redirect-GET 模式）"""
    result_data = session.pop('push_result', None)
    if not result_data:
        return redirect(url_for('index'))

    responses = result_data.get('responses', [])
    sorted_responses = sorted(responses, key=lambda x: (x.get('success', True), x.get('employee_id', '')))

    return render_template('result.html',
        message=result_data.get('message'),
        summary=result_data.get('summary'),
        responses=sorted_responses
    )


@app.route('/history/<int:record_id>')
@login_required
def history_detail(record_id):
    """顯示單一記錄的詳細資訊"""
    records = load_history()
    record = next((r for r in records if r['id'] == record_id), None)
    if not record:
        return "記錄不存在", 404

    record['type_name'] = get_notification_type_name(record.get('notification_type', ''))
    if 'details' in record:
        record['details'] = sorted(record['details'], key=lambda x: (x.get('success', True), x.get('employee_id', '')))

    return render_template('history_detail.html', record=record)

# 取消排程推播
@app.route('/cancel_schedule/<schedule_id>', methods=['POST'])
@login_required
def cancel_schedule(schedule_id):
    """取消排程推播"""
    try:
        try:
            scheduler.remove_job(schedule_id)
            print(f"已從scheduler移除任務: {schedule_id}")
        except Exception as e:
            print(f"移除scheduler任務時出錯（可能已不存在）: {e}")

        remove_scheduled_notification(schedule_id)

        update_schedule_record(schedule_id, {
            'status': 'cancelled',
            'cancelled_at': datetime.now(TAIWAN_TZ).isoformat()
        })

        return jsonify({"success": True, "message": "排程已取消"})
    except Exception as e:
        print(f"取消排程時發生錯誤: {e}")
        return jsonify({"success": False, "error": str(e)}), 500


@app.route('/upload', methods=['POST'])
@login_required
def upload_file():
    print(request.form)
    if 'file' not in request.files:
        return jsonify({"error": "無此檔案"}), 400

    file = request.files['file']
    if file.filename == '':
        return jsonify({"error": "沒有選擇檔案"}), 400

    notification_type = request.form.get("notification_type")
    if not notification_type:
        return jsonify({"error": "請選擇推播類型"}), 400

    if not file.filename.endswith(('.xlsx', '.xls')):
        return jsonify({"error": "檔案必須為excel檔"}), 400

    try:
        excel_data = pd.read_excel(file, sheet_name=None)
        file_title = os.path.splitext(file.filename)[0]

        text_mapping = {
            "etext": "教育補助費已入帳，您的教育補助費總金額為",
            "ftext": "農曆春節禮金已發放，請至薪資帳戶查看，您的春節禮金總金額為"
        }

        if notification_type not in text_mapping:
            return jsonify({"error": "無效的推播類型，請檢查excel格式"}), 400

        body_text = text_mapping[notification_type]
        processed_employees = set()
        notification_tasks = []
        has_valid_sheet = False
        invalid_sheets = []

        for sheet_name, df in excel_data.items():
            if '員工編號' not in df.columns:
                invalid_sheets.append(f"工作表 '{sheet_name}': 缺少「員工編號」欄位")
                continue
            # 確保員工編號是字串格式，去除空白並補零到6位
            df['員工編號'] = df['員工編號'].astype(str).str.strip().str.zfill(6)

            if '合計補助金額' in df.columns:
                amount_column_name = '合計補助金額'
            elif '禮金' in df.columns:
                amount_column_name = '禮金'
            else:
                invalid_sheets.append(f"工作表 '{sheet_name}': 缺少金額欄位\n教育補助需要「合計補助金額」欄位\n春節禮金需要「禮金」欄位")
                continue

            has_valid_sheet = True

            for _, row in df.iterrows():
                employee_id = row['員工編號']
                amount = row.get(amount_column_name)

                if not employee_id or pd.isna(amount):
                    continue
                if employee_id in processed_employees:
                    print(f"跳過重複的員工編號: {employee_id}")
                    continue

                processed_employees.add(employee_id)
                notification_tasks.append({
                    'employee_id': employee_id,
                    'amount': amount,
                    'body_text': body_text,
                    'file_title': file_title
                })

        if not has_valid_sheet:
            error_msg = "Excel 格式不正確！(請參考標準格式範本)\n\n必要欄位：\n1. 員工編號\n2.欄位名稱：「禮金」或「合計補助金額」"
            if invalid_sheets:
                error_msg += "\n\n問題詳情：\n" + "\n".join(invalid_sheets)
            return jsonify({"error": error_msg}), 400

        if not notification_tasks:
            return jsonify({"error": "Excel 中沒有有效的資料（所有員工編號或金額欄位都是空的）"}), 400

        print(f"準備發送 {len(notification_tasks)} 則推播通知...")

        is_scheduled = request.form.get("is_scheduled") == "true"
        schedule_date = request.form.get("schedule_date")
        schedule_time = request.form.get("schedule_time")

        if is_scheduled and schedule_date and schedule_time:
            try:
                schedule_datetime = TAIWAN_TZ.localize(
                    datetime.fromisoformat(f"{schedule_date}T{schedule_time}:00")
                )
                if schedule_datetime <= datetime.now(TAIWAN_TZ):
                    return jsonify({"error": "排程時間必須是未來的時間"}), 400

                schedule_id = f"schedule_{int(datetime.now().timestamp() * 1000)}"

                add_scheduled_notification(
                    schedule_id=schedule_id,
                    notification_type=notification_type,
                    filename=file.filename,
                    schedule_datetime=schedule_datetime,
                    notification_tasks=notification_tasks
                )

                scheduler.add_job(
                    execute_scheduled_notification,
                    trigger=DateTrigger(run_date=schedule_datetime),
                    args=[schedule_id],
                    id=schedule_id,
                    replace_existing=True
                )

                print(f"\n========== 排程推播已建立 ==========")
                print(f"排程ID: {schedule_id}, 預計執行時間: {schedule_datetime}, 任務數: {len(notification_tasks)}")

                ensure_data_directory()
                save_history_record(
                    notification_type=notification_type,
                    filename=file.filename,
                    total=len(notification_tasks),
                    success=0,
                    failed=0,
                    responses=[],
                    is_scheduled=True,
                    scheduled_time=schedule_datetime.isoformat(),
                    schedule_id=schedule_id,
                    operator_name=session.get('display_name', ''),
                    status='pending'
                )

                session['push_result'] = {
                    "message": "推播已排程",
                    "summary": {
                        "scheduled": True,
                        "schedule_time": schedule_datetime.strftime("%Y-%m-%d %H:%M"),
                        "total": len(notification_tasks)
                    },
                    "responses": []
                }
                return redirect(url_for('push_result'))

            except Exception as e:
                print(f"建立排程推播錯誤: {e}")
                return jsonify({"error": f"排程推播錯誤: {e}"}), 500
        else:
            responses = send_bulk(notification_tasks)
            success_count = sum(1 for r in responses if r.get('success', False))
            fail_count = len(responses) - success_count

            print(f"推播完成: 成功 {success_count} 則, 失敗 {fail_count} 則")

            save_history_record(
                notification_type=notification_type,
                filename=file.filename,
                total=len(responses),
                success=success_count,
                failed=fail_count,
                responses=responses,
                operator_name=session.get('display_name', '')
            )

            session['push_result'] = {
                "message": "推播處理完成",
                "summary": {"total": len(responses), "success": success_count, "failed": fail_count},
                "responses": responses
            }
            return redirect(url_for('push_result'))

    except Exception as e:
        print("Error during file processing:", e)
        return jsonify({"error": f"伺服器錯誤: {e}"}), 500


if __name__ == '__main__':
    app.run(host='0.0.0.0', debug=True, port=5002)
